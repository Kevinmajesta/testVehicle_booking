...
class BookingModel extends Model
{
    ...
    {
        return $this->select('bookings.*, vehicles.model as vehicle_name, drivers.name as driver_name, u1.username as approver_1, u2.username as approver_2, users.username as peminjam')
            ->join('vehicles', 'vehicles.id = bookings.vehicle_id', 'left')
            ->join('drivers', 'drivers.id = bookings.driver_id', 'left')
            ->join('users', 'users.id = bookings.user_id', 'left')
            ->join('users u1', 'u1.id = bookings.approver_1_id', 'left')
